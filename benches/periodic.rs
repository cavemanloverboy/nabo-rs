...
fn main() {
    ...
    for run in 0..RUNS {
        ...
        let tree = KDTree::<_,_,D>::new_with_bucket_size(&cloud, 32);
        ...
        println!("querying");
        let timer = Instant::now();
        let non_pbc_knns: Vec<Vec<_>> = (&queries)
            .into_par_iter()
        ...
        let timer = Instant::now();
        let knns: Vec<Vec<_>> = queries
            .into_par_iter()
            .map_with(&tree, |t, query| {
                t.knn_periodic(K, &query, &periodic)
            }).collect::<Vec<Vec<_>>>();
        let query_time = timer.elapsed().as_millis();
        ...
        if run == 0 {
            ...
            for knn in &non_pbc_knns {
                for point in knn {
                    // write buffer to disk
                    file.write_all(&mut point.dist2.to_le_bytes().to_vec()).expect("failed write to disk");
                }
            }   
            ...
            for knn in &non_pbc_knns {
                for point in knn {
                    // write buffer to disk
                    file.write_all(&mut point.index.to_le_bytes().to_vec()).expect("failed write to disk");
                }
            }
